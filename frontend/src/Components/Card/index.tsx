...
interface HeaderProps{
 ...
 const Card:React.FC<HeaderProps> = ({title,img_src,img_alt,price}) => {
    async function handleItens(item:{name:string,img:string,price:number}){
        const atualItens = localStorage.getItem('item')
        const newItens = [...atualItens?JSON.parse(atualItens):JSON.parse('[]'),item]
    ...
    return (
            <div className="card">
                ...
                <button className='bt buy' onClick={()=>{handleItens({name:title,img:img_src,price})}}>COMPRAR</button>
            </div>
    )
}
